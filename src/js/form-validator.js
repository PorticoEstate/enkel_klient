...
const FORM_VALIDATOR_CONFIG = {
  ...
  navigatingAway: false, // Flag to track if user is navigating away from the form
};
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Add navigation detection to prevent validation when navigating away
  setupNavigationDetection();
});

/**
 * Set up event listeners to detect when user is navigating away from the form
 */
function setupNavigationDetection() {
  // Track mousedown (happens before blur) on links and non-submit buttons
  document.addEventListener(
    "mousedown",
    function (e) {
      const target = e.target.closest(
        'a, button:not([type="submit"]), [role="button"], [role="link"]'
      );
      if (target) {
        FORM_VALIDATOR_CONFIG.navigatingAway = true;
        formValidatorLog("Navigation detected via", target.tagName);

        // Reset the flag after a longer delay to ensure it covers the blur event
        setTimeout(() => {
          FORM_VALIDATOR_CONFIG.navigatingAway = false;
        }, 1000);
      }
    },
    true
  ); // Using capturing phase to ensure this runs before other handlers

  // Track keydown for keyboard navigation (Enter or Space on links/buttons)
  document.addEventListener(
    "keydown",
    function (e) {
      if (e.key === "Enter" || e.key === " ") {
        ...
        if (target) {
          FORM_VALIDATOR_CONFIG.navigatingAway = true;
          formValidatorLog("Keyboard navigation detected");
  ...
  // Track beforeunload to handle page navigation
...
function validateField(field) {
  // Skip validation if we're navigating away from the form
  if (FORM_VALIDATOR_CONFIG.navigatingAway) {
    formValidatorLog("Skipping validation because user is navigating away");
    return true;
  }
  ...
  switch (fieldId) {
    ...
    default:
      ...
      if (fieldType === "email" || $field.attr("data-validator") === "email") {
        ...
        fieldType === "date" ||
        $field.hasClass("date-field") ||
        $field.attr("data-validator") === "date"
      ) {
  ...
  if (!isValid) {
    ...
    if (!$errorElement.text()) {
      ...
      // Generate appropriate error message based on field type if no custom message
        ...
        fieldType === "email" ||
        $field.attr("data-validator") === "email"
      ) {
        errorMessage =
          translations?.invalid_email || "Please enter a valid email address";
      } else if (
        fieldId === "phone" ||
        fieldId === "mobile" ||
        fieldId === "telephone" ||
        ...
        errorMessage =
          translations?.invalid_postal || "Please enter a valid postal code";
      } else if ($field.prop("required")) {
        errorMessage = translations?.field_required || "This field is required";
      } else {
        errorMessage =
          translations?.invalid_input || "Please enter valid information";
      }
...
function validateAllFields(form) {
  ...
  if (!allValid) {
    ...
    formValidatorLog(
      `Form validation failed with ${errors.length} errors`,
...
function validateForm(formElement) {
  ...
  formValidatorLog(
    "validateForm called",
    formElement ? formElement.id || "with element" : "without element"
  );
  ...
  if (
    ...
    (formElement.tagName === "FORM" ||
      (formElement.jquery && formElement[0].tagName === "FORM"))
  ) {
    formValidatorLog(
      "Using provided form element",
      formElement.id || "unnamed"
    );
  ...
  formValidatorLog(
    "Falling back to first form in document",
    firstForm.attr("id") || "unnamed"
  );
...
function setupFormValidation(form) {
  ...
  $form
    ...
    .on("blur", function (e) {
      // Delay validation slightly to allow navigation flag to be set
      setTimeout(() => {
        // Skip validation if navigating away
        if (FORM_VALIDATOR_CONFIG.navigatingAway) {
          formValidatorLog(
            "Skipping blur validation because user is navigating away"
          );
          return;
        }
        validateField($(this));
...
function displayErrorSummary($form, errors) {
  ...
  if ($errorSummary.length === 0) {
    $errorSummary = $(
      '<div class="error-summary alert alert-danger" role="alert" aria-labelledby="error-summary-title"></div>'
    );
  ...
  let html =
    '<h2 id="error-summary-title" class="h5">' +
  ...
  if (typeof announceToScreenReader === "function") {
    ...
    const message =
      errorCount === 1
        ...
        : errorCount +
          " form errors found. Please correct the highlighted fields.";
